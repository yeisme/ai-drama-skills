...
1. 读取 Episode/Scene/Shot refs、DirectorDecisionGraph、`ShotAudioIntent`、dialogue、duration、provider audio policy 和 subtitle constraints；字段合同见 `../ai-drama-router/references/shot-audio-intent-contract.md`。
...
4. 若视频包含 provider 原生音轨，先按 `none | external_assets | video_native | replace_after_generation` 分类。默认把不可控原生音频登记为 `video_native_audio` + `pending_review`，不得静默进入最终 mix。
