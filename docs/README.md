...
本模块把剧型策略、上下文包、导演、编剧、人物、连续性、评估和制片判断拆成可路由的第一方 Skills。用户入口保持为 `ai-drama-router`，专项 Skills 默认按需加载。对已有剧本的评估必须先经过 `ai-drama-assessment` 建立题材化 `AssessmentContract`，再进入 `writing/ai-drama-critic-review` 模板；没有合同不得给综合分。用户所说的“AI味”拆为对白、叙述/动作和结构模板三条自然度/模式风险 lane，不输出作者来源概率；媒介标签只有用户明示时才冻结。
...
- 已收编：`writing/ai-drama-format-strategy` 模板（原 format-strategist 参考已并入模板正文）
